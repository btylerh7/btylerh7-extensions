...
class MangaGohan extends paperback_extensions_common_1.Source {
    constructor() {
        super(...arguments);
        this.parser = new MangaGohanParser_1.Parser();
        this.cookies = [
            createCookie({
                name: 'isAdult',
                value: '1',
...
class Parser {
    ...
    parseMangaDetails($, mangaId) {
        ...
        const status = paperback_extensions_common_1.MangaStatus.UNKNOWN; //All manga is listed as ongoing
    ...
    parseChapters($, mangaId) {
        ...
        for (const href of chapterLinks.toArray()) {
    ...
    parseHomeSections($, sectionCallback) {
        ...
        for (const featuredManga of $('.popular-slider.style-1').find('.slider__item').toArray()) {
        ...
        for (const topManga of $('.c-blog__content').first().find('.page-item-detail.manga').toArray()) {
    ...
    parseTags($) {
        ...
        for (const tag of otherIdentifiedTags) {
